...
static void velog(int pri, const char *file,const int line,char *fmt,va_list ap);
...
void elog(int pri, const char *file,const int line,char *fmt,...)
{
   if ( pri > LOG_LEVEL_DEBUG || pri < LOG_LEVEL_ERROR )
   {
      pri = LOG_LEVEL_ERROR;
...
static char   fileline[2048];  /*原 1024*/
...
static void velog(int pri, const char *file, const int line, char *fmt, va_list ap)
{
   ...
   */
    ...
    xlen = snprintf(p, llen, "[%s:%d] [%s:%d]  ", ( edata.module != NULL ) ? edata.module : "unkown", 
